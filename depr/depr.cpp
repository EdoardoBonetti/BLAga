                                                        ...
                                                        size_t M, size_t N, size_t K)
{
    ...
    for (; k + UNROLL <= K; k += UNROLL)
    {
        for (size_t u = 0; u < UNROLL; ++u)
        {
            ...
            if (N >= 1)
                c0 = _mm256_fmadd_pd(a_vec, _mm256_set1_pd(packedB[0 * K + k + u]), c0);
            if (N >= 2)
                c1 = _mm256_fmadd_pd(a_vec, _mm256_set1_pd(packedB[1 * K + k + u]), c1);
            if (N >= 3)
                c2 = _mm256_fmadd_pd(a_vec, _mm256_set1_pd(packedB[2 * K + k + u]), c2);
            if (N >= 4)
                c3 = _mm256_fmadd_pd(a_vec, _mm256_set1_pd(packedB[3 * K + k + u]), c3);
        }
    }
    ...
    for (; k < K; ++k)
    {
        ...
        if (N >= 1)
            c0 = _mm256_fmadd_pd(a_vec, _mm256_set1_pd(packedB[0 * K + k]), c0);
        if (N >= 2)
            c1 = _mm256_fmadd_pd(a_vec, _mm256_set1_pd(packedB[1 * K + k]), c1);
        if (N >= 3)
            c2 = _mm256_fmadd_pd(a_vec, _mm256_set1_pd(packedB[2 * K + k]), c2);
        if (N >= 4)
            c3 = _mm256_fmadd_pd(a_vec, _mm256_set1_pd(packedB[3 * K + k]), c3);
    }
    ...
    size_t M, size_t N, size_t K)
{
    ...
    for (size_t k = 0; k < K; ++k)
    {
        ...
        // FMA with packedB
        if (N >= 1)
            c0.fmadd_scalar(a_vec, packedB[0 * K + k]);
        if (N >= 2)
            c1.fmadd_scalar(a_vec, packedB[1 * K + k]);
        if (N >= 3)
            c2.fmadd_scalar(a_vec, packedB[2 * K + k]);
        if (N >= 4)
            c3.fmadd_scalar(a_vec, packedB[3 * K + k]);
    }
    ...
    size_t M_block, size_t N_block, size_t K_block)
{
    ...
    for (size_t ii = 0; ii < M_block; ii += 4)
    {
        ...
        for (size_t jj = 0; jj < N_block; jj += 4)
        {
            ...
            (aptr, bptr, cptr, lda, ldc, Mtail, Ntail, K_block);
        }
    }
