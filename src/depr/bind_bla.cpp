...
PYBIND11_MODULE(bla, m)
{
     ...
     py::class_<Matrix<double>>(m, "Matrix")
         ...
         .def("__getitem__", [](Matrix<double> &m, std::pair<int, int> idx)
              {
          int i = idx.first < 0 ? idx.first + m.nRows() : idx.first;
          int j = idx.second < 0 ? idx.second + m.nCols() : idx.second;
          return m(i, j); })
         .def("__setitem__", [](Matrix<double> &m, std::pair<int, int> idx, double val)
              {
          int i = idx.first < 0 ? idx.first + m.nRows() : idx.first;
          int j = idx.second < 0 ? idx.second + m.nCols() : idx.second;
          m(i, j) = val; })
         ...
         .def(py::pickle(
             [](Matrix<double> &m)
             {
                  return py::make_tuple(m.nRows(), m.nCols(),
                                        py::bytes((char *)m.Data(), m.nRows() * m.nCols() * sizeof(double)));
             },
             [](py::tuple t)
             {
                  size_t rows = t[0].cast<size_t>();
                  size_t cols = t[1].cast<size_t>();
                  Matrix<double> m(rows, cols);
                  py::bytes mem = t[2].cast<py::bytes>();
                  std::memcpy(m.Data(), PYBIND11_BYTES_AS_STRING(mem.ptr()), rows * cols * sizeof(double));
                  return m;
             }));
}