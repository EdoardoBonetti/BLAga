...
namespace bla_ga
{
  ...
  template <typename T, typename S>
  auto operator*(const Vector<T> &a, const Vector<S> &b) -> decltype(std::declval<T>() * std::declval<S>())
  {
    typedef decltype(std::declval<T>() * std::declval<S>()) U;
    U sum(0);
    for (size_t i = 0; i < a.Size(); i++)
  ...
  template <typename T, typename S>
  auto operator*(const T &t, const Vector<S> &v) -> Vector<decltype(std::declval<T>() * std::declval<S>())>
  {
    typedef decltype(std::declval<T>() * std::declval<S>()) U;
    Vector<U> sum(v.Size());
